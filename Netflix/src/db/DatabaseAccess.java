...
public class DatabaseAccess extends Thread {
	...
	// JDBC URLs
	static String urlPSQL = "jdbc:postgresql://192.168.2.22:5432/netflix";
			...
			if (jedis.lrange(Integer.toString(querySQL.hashCode()), 0, -1).size() == 0) {
				...
				while (rs.next()) {
					for(int i = 1; i <= numcolum ;i++) {
						...
						movies.add(add);
						jedis.rpush(Integer.toString(querySQL.hashCode()), add);
					}
				}
				...
				movies = (ArrayList<String>)jedis.lrange(Integer.toString(querySQL.hashCode()), 0, -1);
